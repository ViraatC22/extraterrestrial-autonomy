...
soft soil. What makes this failure mode interesting for autonomy research is
...
rovers rather than slope alone; Angelova et al. learned to predict slip from
terrain appearance, and Helmick et al. built terrain-adaptive navigation
around slip-aware models. More recent work makes the uncertainty explicit
rather than producing a single traversability score: Endo et al. fuse
probabilistic traversability predictions for risk-aware planning on
heterogeneous terrain, and extend this with test-time adaptation so that a
traversability model can be corrected during deployment. Sanchez-Ibanez et al.
...
paired, so it uses McNemar's exact test, which conditions on the seeds where
the two planners actually disagreed. Holm--Bonferroni correction is applied
...
\subsection{Degraded conditions: H3 partially supported}

The one contrast surviving Holm correction across the ten-member primary family
is mission success under injected hardware faults, where adaptation raised
success from \MainMarsFaultFixedSuccess{} to \MainMarsFaultAdaptiveSuccess{}
($\Delta = \MainDeltaMarsFaultSuccess$,
$p_{\mathrm{Holm}} = \MainPMarsFaultSuccess$). The discordant pairs are
one-sided: of the ten seeds where the two planners disagreed, the adaptive
planner succeeded on all ten and the fixed planner on none.

The other two degraded conditions move the same way without reaching
corrected significance: $\MainDeltaMarsUncSuccess$ under elevated slip
dispersion (12 of 13 discordant seeds favouring adaptation,
$p_{\mathrm{Holm}} = \MainPMarsUncSuccess$) and $\MainDeltaMarsCommSuccess$
under intervention delay ($p_{\mathrm{Holm}} = \MainPMarsCommSuccess$). The
direction is consistent across all three degraded conditions, which is
suggestive, but only one is individually defensible after correction and we do
not claim more.

\subsection{Adaptation buys survival, not science}

Across every Martian condition the sign pattern is the same: adaptation
improves mission success while slightly \emph{reducing} science fraction
(for example $\MainDeltaMarsCommScience$ under intervention delay). The
mechanism is visible in the termination counts. Under faults, energy
exhaustion falls from \MainMarsFaultFixedEnergyOut{} missions to
\MainMarsFaultAdaptiveEnergyOut{}, and ground interventions fall from
\MainMarsFaultFixedInterventions{} to \MainMarsFaultAdaptiveInterventions{}
per mission.

This follows directly from Equation~\ref{eq:energy}. A robot that has measured
that the local ground is slippier than its prior assumed raises its estimate of
what every remaining route will cost, and therefore turns back sooner. Turning
back sooner is exactly what converts a stranded rover into a surviving one ---
and exactly what costs it the last science target. The adaptive planner is not
better at everything; it is more accurate about its own energy budget, and
accuracy here purchases caution.
...
\caption{How missions ended. The effect of adaptation appears as a shift
between termination causes -- fewer energy exhaustions -- rather than as a
uniform improvement.}
\end{figure}
...
appearing clearly during tuning. What survived confirmatory analysis is
narrower and different in kind --- adaptation improved \emph{survival} under
hardware faults, and did so by making the robot's energy predictions honest
enough that it turned back in time.

Three things follow.

First, the value of online adaptation in this testbed is concentrated where
the robot's own state is degrading, not merely where the terrain is
unfamiliar. Faults change the vehicle underneath the model; a model that
cannot be revised keeps planning for a rover that no longer exists.
...
\begin{enumerate}[leftmargin=2em]
  \item A. Angelova, L. Matthies, D. Helmick, G. Sibley, P. Perona.
  ``Learning to Predict Slip for Ground Robots.'' ICRA, 2006.
  \item D. Helmick, A. Angelova, M. Livianu, L. Matthies. ``Terrain Adaptive
  Navigation for Mars Rovers.'' IEEE Aerospace Conference, 2007.
  \item M. Endo, T. Taniai, R. Yonetani, G. Ishigami. ``Risk-aware Path
  Planning via Probabilistic Fusion of Traversability Prediction for Planetary
  Rovers on Heterogeneous Terrains.'' ICRA, 2023. arXiv:2303.01169.
  \item M. Endo, T. Taniai, G. Ishigami. ``Deep Probabilistic Traversability
  with Test-time Adaptation for Uncertainty-aware Planetary Rover
  Navigation.'' 2024. arXiv:2409.00641.
  \item J. R. Sanchez-Ibanez, P. J. Sanchez-Cuevas, M. A. Olivares Mendez.
  ``Optimal and Risk-Aware Path Planning considering Localization Uncertainty
  for Space Exploration Rovers.'' IROS, 2022.
  \item NASA. ``Spirit.'' Mission page documenting the 2009 loss of traction
  in soft soil.
\end{enumerate}
