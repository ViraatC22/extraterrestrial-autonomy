...
# No timestamps in figure files, so a rebuild from the same data is byte-identical.
_METADATA = {".pdf": {"CreationDate": None, "ModDate": None}, ".png": {"Software": None}}
...
def write_outcome_figure(df: pd.DataFrame, stem: Path) -> None:
    ...
    for suffix in (".pdf", ".png"):
        fig.savefig(stem.with_suffix(suffix), dpi=180, metadata=_METADATA[suffix])
    plt.close(fig)
...
def main() -> None:
    ...
    for family, frame in (("primary", primary), ("secondary", secondary)):
        if frame.empty:
            continue
        write_contrast_table(frame, TABLE_DIR / f"exonaut_{tag}_{family}.tex", family)
        committed = RESULTS_DIR / f"exonaut_{tag}_{family}.csv"
        if tag == "main" and committed.exists():
            # Study 1's contrast tables are locked (data/results/v1_LOCK.json):
            # recompute and check, never overwrite.
            old = pd.read_csv(committed)
            numeric = old.select_dtypes("number").columns
            pd.testing.assert_frame_equal(
                old[numeric].reset_index(drop=True),
            ...
            frame.to_csv(committed, index=False)
