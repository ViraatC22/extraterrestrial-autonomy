...
def _metadata(
    ...
    return {
        **environment_record(design_name, design),
        "schema_version": 1,
...
def run_mission_sweep(
    ...
    df = pd.DataFrame(rows).sort_values(["condition", "planner", "seed"]).reset_index(drop=True)
